/*******************************************************************************************
 *  @file      TwoSumSolution.cpp 2015\12\14 14:10:21 $
 *  @author    Wang Xiaotao<wangxiaotao1980@gmail.com> (中文编码测试)
 *  @note      LeetCode No.1 Two Sum
 *******************************************************************************************/

#include "TwoSumSolution.hpp"
...
std::vector<int> TwoSumSolution::twoSum(std::vector<int>& nums, int target)
{
    ...
    for (std::unordered_multimap<int, int>::const_iterator cit = theMap.cbegin(); cit != theMap.cend(); ++cit)
    {
        ...
        if (theOther == cit->first)
        {
            ...
            for (std::unordered_multimap<int, int>::const_iterator cit = rangeIter.first; cit != rangeIter.second;
                 ++cit)
            {
